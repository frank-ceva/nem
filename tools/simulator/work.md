...
- Use registry data for opcode dispatch and attribute validation.
- Use `type_families` references for type legality checks against simulated device config.
- Use `execution_unit` field for simulation routing.
...
1. **New opcode simulation**: Simulate `layernorm`, `rmsnorm`, `softmax`, `log_softmax`, `gelu`, `silu` operations.
2. **INT4 simulation**: Support i4 × i8 mixed-precision GEMM and Conv2D.
3. **Per-group dequantization**: Support per-group quant descriptor in simulation.
4. **Device model**: Model sDMA/WDM/VPU/SEQ units, memory capacities, and unit_characteristics in timing simulation.
...
The NEM spec now includes `const` declarations (see `spec/CHANGELOG.md`). The simulator must support:
...
4. **Simulation**: Substitute constant values in all expression evaluation contexts (buffer sizes, region offsets, shapes, loop bounds).
